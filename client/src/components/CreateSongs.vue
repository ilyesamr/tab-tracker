<template>
  ...
  <label >Lyrics : </label>
  <textarea required :rules="[required]" class="form-control" rows="3" name="lyrics" v-model="song.lyrics"></textarea>
</div>
  <div class="form-group green-border-focus">
  <label>Tab : </label>
  <textarea required :rules="[required]" class="form-control" rows="3" name="tab" v-model="song.tab"></textarea>
    ...
    <br>
   <button @click="create" class="btn btn-success">Créer la musique</button>
        <br>
